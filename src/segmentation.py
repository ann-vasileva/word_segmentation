...
def segment_text_fast(text, trie, bigram_probs, uniram_probs, max_word_len=30):
    raw_text, posit = remove_punctuation(text)
    raw_text = split_on_heuristics(raw_text)
    ...
    for seg in segments:
        ...
        for i in range(n):
            if dp[i] == -float('inf'):
                continue
            node = trie
            for j in range(i, min(n, i + max_word_len)):
                ch = seg[j].lower()
                if ch == "ё":
                    ch = "е"
                ...
                if node.is_word:
                    ...
                    bigram_score = bigram_probs.get((prev_word[i], word), -25.0) if i > 0 else uniram_probs.get(word, -25.0)
